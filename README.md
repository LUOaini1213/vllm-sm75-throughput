...
### （2026-09-08）干预实验做完了，根因坐实

三组配置只差一个参数，判据跑前写死。**三条预测全中，无一需要事后改口径。**

| TPOT p50 (ms) | 4 | 8 | 12 | 16 | 24 | 32 | 捕获的 bs |
|---|---|---|---|---|---|---|---|
| A 默认 | 5.70 | 6.29 | **34.27** | 35.00 | 32.80 | 35.74 | `[1,2,4,8]` |
| B 关解码图 | 34.17 | 32.32 | 33.77 | 33.91 | 34.15 | 36.31 | 无 |
| C 上限抬到 32 | 5.85 | 7.88 | **7.52** | 7.74 | 9.71 | 10.26 | `[1,2,4,8,12,16,24,32]` |

**B 组是决定性的那一条**，因为关图代价按「原本有没有图」精确分裂：

| batch | 4 | 8 | 12 | 16 | 24 | 32 |
|---|---|---|---|---|---|---|
| B ÷ A | **6.00×** | **5.14×** | 0.99× | 0.97× | 1.04× | 1.02× |
| A 组有图吗 | 有 | 有 | 没有 | 没有 | 没有 | 没有 |

原本有图的档关掉后慢 5 到 6 倍，原本就没图的档纹丝不动。
关掉一个机制，只有依赖它的部分受影响。**这是因果，不是相关。**
...
|---|---|---|
| A 默认 | 2.36 GB | 4 |
| C 抬上限 | **2.36 GB（与 A 相同）** | **8** |
| D 默认后端 | 1.91 GB | 4 |

同显存不同桶数，不同显存同桶数。显存既非充分也非必要条件。

**「未测」也关闭了**：默认 flashinfer 后端这次 **248s 就绪**（上轮 420s 超时是时间不够，不是不兼容），
台阶在同一位置（6.33 → 29.92 ms）。所以结论**不是 Triton + PyTorch 次优配置的产物**。

**一个可直接用的优化**：T4 上只要并发可能超过 8，就该显式设 `--cuda-graph-max-bs-decode`。
框架档位表按「T4 是小卡」给了 8，实测显存完全够捕到 32。

| 吞吐提升（C ÷ A） | batch 12 | batch 16 | batch 24 | batch 32 |
|---|---|---|---|---|
| | 4.05× | 4.12× | 3.18× | **4.42×** |

**仍未做**：vLLM 侧反向复现（把捕获桶砍到 `[1,2,4,8]`）。在跑出来之前只能说
「这是 SGLang 上的因果实证」，**不能说「这是图捕获分桶的通用机制」**。
另有一处未解释：C 组 batch 8 反而比 A 慢（7.88 vs 6.29 ms），单次测量，未复现，不下结论。
...
→ 因果实证原始数据：[`results_t4/dip_causal_2026-09-08.txt`](results_t4/dip_causal_2026-09-08.txt)
→ 两条更正的完整依据：[`results_t4/corrections_2026-09-07.txt`](results_t4/corrections_2026-09-07.txt)
（旧结果文件一律不改、留档，更正另立新文件）

## 复现

八份 notebook 可直接在 Colab（T4）打开跑（最后一份 `cloud_cross_vllm_graph` 已自检，**尚未跑出结果**），每份第 1 节都列了该框架的环境阻塞与修法：
...
2026-09-07 做了一轮上游调研，产出 36 条候选并分四层排序，见
[`docs/下一步_优化方向调研_2026-09-07.md`](docs/%E4%B8%8B%E4%B8%80%E6%AD%A5_%E4%BC%98%E5%8C%96%E6%96%B9%E5%90%91%E8%B0%83%E7%A0%94_2026-09-07.md)。
这轮调研的直接产物就是上面那两条更正 —— **不是新实验查出来的，是去读上游源码查出来的**。

报告里也写明了在 sm_75 上**做不了**的方向及其依据（KV cache fp8、FlashAttention、
SGLang 投机解码的 sm_75 内核缺失等），以及哪些结论我打开源码核过、哪些只是检索报告。
检索原始输出留在 [`docs/_调研候选36条_2026-09-07.md`](docs/_%E8%B0%83%E7%A0%94%E5%80%99%E9%80%8936%E6%9D%A1_2026-09-07.md)，
**未经逐条核验，结论以报告为准**。
